...
//    class WC
//    {
//        static void Main(string[] argsFromFile)
//        {
//            if (CommandLine.ParseArgumentsWithUsage(argsFromFile, parsedArgs))
//            {
...
namespace ARMSim.CommandLine
{
    ...
    public sealed class Parser
    {
        ...
        /// <param name="reporter">reporter to write errors to</param>
...
//                System.Console.Write(Parser.ArgumentsUsage(destination.GetType()));
        ...
        /// </summary>
        /// <param name="argsFromFile"> Args to check for help. </param>
        /// <returns> Returns true if argsFromFile contains /? or /help. </returns>
        ...
        /// </summary>
        /// <param name="argsFromFile"> The arguments to parse. </param>
        ...
        private ArgumentHelpStrings[] GetAllHelpStrings()
        {
            ...
            strings[index++] = new ArgumentHelpStrings("@<file>", "Read arguments from a file");
        ...
        {
            string argsFromFile  = null;
                    
            try
            {
                using (FileStream file = new FileStream(fileName, FileMode.Open, FileAccess.Read))
                {
                    argsFromFile = (new StreamReader(file)).ReadToEnd();
                }
            }
            catch (Exception e)
            {
            ...
            // while (index < argsFromFile.Length)
            try
            {
                while (true)
                {
                    // skip whitespace
                    while (char.IsWhiteSpace(argsFromFile[index]))
                    {
                        index += 1;
                    }
                    
                    // # - comment to end of line
                    if (argsFromFile[index] == '#')
                    {
                        index += 1;
                        while (argsFromFile[index] != '\n')
                        {
                            index += 1;
                        }
                        continue;
                    ...
                    do
                    {
                        if (argsFromFile[index] == '\\')
                        {
                            int cSlashes = 1;
                            index += 1;
                            while (index == argsFromFile.Length && argsFromFile[index] == '\\')
                            {
                                cSlashes += 1;
                            }

                            if (index == argsFromFile.Length || argsFromFile[index] != '"')
                            {
                        ...
                        {
                            currentArg.Append(argsFromFile[index]);
                            index += 1;
                        }
                    } while (!char.IsWhiteSpace(argsFromFile[index]) || inQuotes);
                ...
                get
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            builder.Append(":{");
                            bool first = true;
                            foreach (FieldInfo fld in valueType.GetFields())
                            {
                                if (fld.IsStatic)
                                {
                                    if (first)
